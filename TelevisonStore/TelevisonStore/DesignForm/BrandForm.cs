...
namespace TelevisonStore.DesignForm
{
    public partial class BrandForm : UserControl
    {
        ...
        public void setData(int pageNumber)
        {
            ...
            string sql = "select * from (select ROW_NUMBER() over (order by iD) as STT,Id ,Code as N'Mã',Name as N'Tên',iif(Status=1,N'Hoạt động',N'Không hoạt động') as N'Trạng thái' from Brand) as abc where abc.STT >" +skip+ "and abc.STT <=" + take ;
            string count = "select count(*) from Brand";
            // nếu có điều kiện thì thêm cả điều kiện
            if (search != "")
            {
                sql = "select * from (select ROW_NUMBER() over (order by iD) as STT,Id ,Code as N'Mã',Name as N'Tên',iif(Status=1,N'Hoạt động',N'Không hoạt động') as N'Trạng thái' from Brand where code like N'%"+search+ "%' or name like N'%" + search + "%' ) as abc where abc.STT >" + skip + "and abc.STT <=" + take;
                count = "select count(*) from Brand where code like N'%"+search+ "%' or name like N'%" + search + "%'";
            }
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            if (generalInform.checkTextBoxInForm(panelUpdateData))
            {
                ...
                // insert && update table
                string sql = "insert into brand(code,name,status) values ('" + code + "',N'" + name + "','" + status + "')";
                if (!id.Equals("0"))
                {
                    sql = "update Brand set code =N'" + code + "', name = N'" + name + "', status = '" + status + "' where id = '" + id + "'";
                }
                con.UpdateData(sql);
                setData(pageIndex);
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (open.getCheck())
            {
                int index = dataGridView.CurrentRow.Index;
                int id = (int)dataGridView.Rows[index].Cells[1].Value;
                con.UpdateData("delete Brand where Id = " + id);
                setData(pageIndex);
                open.Dispose();
            }
            else
