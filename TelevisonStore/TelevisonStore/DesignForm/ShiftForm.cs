...
{
    public partial class ShiftForm : UserControl
    {
        GeneralInForm generalInform;
        ConnectDatabase con;

        // phân trang
        int pageShift = 10;   // số lượng bản ghi trên 1 trang
        ...
        {
            // phân trang
            string skip = (pageShift * (pageNumber - 1)).ToString();
            string take = ((pageShift * (pageNumber - 1)) + pageShift).ToString();
            string search = textBoxSearch.Text.ToString();

            // khởi tạo câu lệnh truy vấn
            string sql = "select * from (select ROW_NUMBER() over (order by iD) as STT,Id ,Code as N'Mã',Name as N'Tên',iif(Status=1,N'Hoạt động',N'Không hoạt động') as N'Trạng thái' from Shift) as abc where abc.STT >" + skip + "and abc.STT <=" + take;
            string count = "select count(*) from Shift";
            // nếu có điều kiện thì thêm cả điều kiện
            if (search != "")
            {
                sql = "select * from (select ROW_NUMBER() over (order by iD) as STT,Id ,Code as N'Mã',Name as N'Tên',iif(Status=1,N'Hoạt động',N'Không hoạt động') as N'Trạng thái' from Shift where code like N'%" + search + "%' or name like N'%" + search + "%' ) as abc where abc.STT >" + skip + "and abc.STT <=" + take;
                count = "select count(*) from Shift where code like N'%" + search + "%' or name like N'%" + search + "%'";
            }

            // lấy tổng số lượng bản ghi
            int total = con.GetTotal(count);
            // chia lấy xem có bao nhiêu trang 
            pageCount = total % pageShift == 0 ? total / pageShift : total / pageShift + 1;
            // set data
        ...
        {
            generalInform.FocusTextBox(textBoxCode, panelCode, "Nhập mã ca");
        }

        private void textBoxCode_Leave(object sender, EventArgs e)
        {
            generalInform.LeaveTextBox(textBoxCode, panelCode, "Nhập mã ca");
        }

        private void textBoxName_MouseClick(object sender, MouseEventArgs e)
        {
            generalInform.FocusTextBox(textBoxName, panelName, "Nhập tên ca");
        }

        private void textBoxName_Leave(object sender, EventArgs e)
        {
            generalInform.LeaveTextBox(textBoxName, panelName, "Nhập tên ca");
        }
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            if (generalInform.checkTextBoxInForm(panelUpdateData))
            {
                ...
                // insert && update table
                string sql = "insert into Shift(code,name,status) values ('" + code + "',N'" + name + "','" + status + "')";
                if (!id.Equals("0"))
                {
                    sql = "update Shift set code =N'" + code + "', name = N'" + name + "', status = '" + status + "' where id = '" + id + "'";
                }
                con.UpdateData(sql);
                setData(pageIndex);
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (open.getCheck())
            {
                int index = dataGridView.CurrentRow.Index;
                int id = (int)dataGridView.Rows[index].Cells[1].Value;
                con.UpdateData("delete Shift where Id = " + id);
                setData(pageIndex);
                open.Dispose();
            }
            else
