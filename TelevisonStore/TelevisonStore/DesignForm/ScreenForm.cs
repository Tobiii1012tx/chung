...
{
    public partial class ScreenForm : UserControl
    {
        ...
        public void setData(int pageNumber)
        {
            ...
            string sql = "select * from (select ROW_NUMBER() over (order by iD) as STT,Id ,Code as N'Mã' ,Name as N'Tên',iif(Status=1,N'Hoạt động',N'Không hoạt động') as N'Trạng thái' from Screen) as abc where abc.STT >" + skip + "and abc.STT <=" + take;
            string count = "select count(*) from Screen";
            // nếu có điều kiện thì thêm cả điều kiện
            if (search != "")
            {
                sql = "select * from (select ROW_NUMBER() over (order by iD) as STT,Id ,Code as N'Mã',Name as N'Tên',iif(Status=1,N'Hoạt động',N'Không hoạt động') as N'Trạng thái' from Screen where code like N'%" + search + "%' or name like N'%" + search + "%' ) as abc where abc.STT >" + skip + "and abc.STT <=" + take;
                count = "select count(*) from Screen where code like N'%" + search + "%' or name like N'%" + search + "%'";
            }
        ...
        {
            generalInform.FocusTextBox(textBoxCode, panelCode, "Nhập mã màn hình");
        }

        private void textBoxCode_Leave(object sender, EventArgs e)
        {
            generalInform.LeaveTextBox(textBoxCode, panelCode, "Nhập mã màn hình");
        }

        private void textBoxName_MouseClick(object sender, MouseEventArgs e)
        {
            generalInform.FocusTextBox(textBoxName, panelName, "Nhập tên màn hình");
        }

        private void textBoxName_Leave(object sender, EventArgs e)
        {
            generalInform.LeaveTextBox(textBoxName, panelName, "Nhập tên màn hình");
        }
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            if (generalInform.checkTextBoxInForm(panelUpdateData))
            {
                ...
                // insert && update table
                string sql = "insert into Screen(code,name,status) values (N'" + code + "',N'" + name + "','" + status + "')";
                if (!id.Equals("0"))
                {
                    sql = "update Screen set code =N'" + code + "', name = N'" + name + "', status = '" + status + "' where id = '" + id + "'";
                }
                con.UpdateData(sql);
                setData(pageIndex);
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (open.getCheck())
            {
                int index = dataGridView.CurrentRow.Index;
                int id = (int)dataGridView.Rows[index].Cells[1].Value;
                con.UpdateData("delete Screen where Id = " + id);
                setData(pageIndex);
                open.Dispose();
            }
            else
